...
// The items currently located in the history are not free.
// However, they don't cause memory leak because the size is not accumulated (fixed size)
...
void sushi_store(char *line) {
	...
	// DZ: Changed >= to >
	// DZ: Why didn't you fix the problem?
